...
YUI().use('node', 'substitute', 'cookie', "event-resize", "transition", "event", 'jsonp', 'json-parse', function(Y) {
  ...
  core.setupAnimations = function() {
    ...
    if (Y.one('body').hasClass('phone') || Y.one('body').hasClass('phone-scopes')) {
      ...
      if (Y.one('.search-screen')) {
        var searchScreen = {
          ypos: Y.one('.search-screen').getXY()[1] - yOffset,
          run: false
        };
      }
        ...
        edgeMagic.each(function(node) {
          if (window.scrollY > node.getXY()[1] - yOffset && window.scrollY < node.getXY()[1] && !node.run) {
            ...
            if (node.one('.slider-animation').getAttribute('class') == 'slider-animation full-swipe run') {
              setTimeout(function() {
                node.one('.launcher').addClass('return')
              }, 2000);
            }
          }
        });
  ...
  core.runAnimation = function($anim) {
    switch ($anim) {
      case 'search-screen':
        ...
        setTimeout(function() {
          Y.one('.search-screen').removeClass('run');
        }, 2000);
        break;
      case 'go-back':
  ...
  core.updateSlider = function($index) {
    ...
    YUI().use('node', function(Y) {
      ...
      switch ($index + '') {
        case '0':
          setTimeout(function() {
            Y.one('.edge-magic').addClass('run');
          }, 1200);
          break;
        case '1':
          setTimeout(function() {
            Y.one('.full-swipe').addClass('run');
          }, 1200);
          setTimeout(function() {
            Y.one('.full-swipe .launcher').addClass('return')
          }, 2000);
          break;
        case '2':
          setTimeout(function() {
        ...
        case '3':
          setTimeout(function() {
            Y.one('.content-controls').addClass('run');
          }, 1200);
          break;
      }
    });
  ...
  core.rerunAnimation = function($type) {
    ...
    if ($type == 'full-swipe') {
      ...
      Y.one('.' + $type).removeClass('run');
      setTimeout(function() {
        Y.one('.' + $type).addClass('run');
      }, 1000);
    } else if ($type == 'slider-animation') {} else {
      Y.one('.' + $type).removeClass('run');
      setTimeout(function() {
        Y.one('.' + $type).addClass('run');
      }, 400);
  ...
  core.renderJSON = function(response, id) {
    ...
    var numberToDisplay = numberPartners < 10
      ? numberPartners
      : 10;
